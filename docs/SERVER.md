...
Set `GOVERNANCE_ENGINE=pi` to use the alternate Pi SDK reviewer. It creates an ephemeral in-memory session in a temporary empty directory, disables extensions, skills, prompt templates, themes, context discovery, compaction, and retry, and allowlists only the strict `submit_review` custom tool. The Pi source is pinned as the `vendor/pi` submodule while the matching published package supplies the runtime build. Pi receives no GitHub credential or write tool; the existing deterministic service remains the only GitHub publisher.

On the first governance request for a pinned integration SHA, either engine collects a bounded complete text-code snapshot, then derives a Markdown repository-context document from the full Git tree/file map, source excerpts, manifests, root context documents, and scoped-policy map. A new integration SHA regenerates that document; the current PR patch remains separate projected evidence rather than silently becoming baseline truth. A Human Developer can add or correct an advisory note through `PUT /api/governance/context` using the returned baseline SHA and revision. The context document, user note, policies, and PR diff are all included in the review digest, so concurrent changes invalidate the result.
